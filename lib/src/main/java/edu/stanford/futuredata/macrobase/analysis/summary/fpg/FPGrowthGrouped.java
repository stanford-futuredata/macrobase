...
//        }
        ...
        public void insertFrequentItems(List<ItemsetWithCount> transactions,
                                        int countRequiredForSupport) {

            Map<Integer, Double> itemCounts = new HashMap<>();
            for (ItemsetWithCount iSet : transactions) {
                Set<Integer> t = iSet.getItems();
                double curCount = iSet.getCount();
                for (Integer item : t) {
                    itemCounts.compute(item, (k, v) -> v == null ? curCount : v + curCount);
                }
            }
        ...
        public void insertTransactions(List<ItemsetWithCount> transactions) {

            for (ItemsetWithCount iSet : transactions) {
                Set<Integer> t = iSet.getItems();
                double count = iSet.getCount();
                List<Integer> filtered = t.stream().filter(i -> frequentItemCounts.containsKey(i)).collect(
                        Collectors.toList());

                if (!filtered.isEmpty()) {
                    filtered.sort((i1, i2) -> frequentItemOrder.get(i2).compareTo(frequentItemOrder.get(i1)));
                    root.insertTransaction(filtered, 0, count);
                }
            }
        }
    ...
    public List<ItemsetWithCount> getItemsetsWithSupportRatio(List<ItemsetWithCount> transactions,
                                                              Double supportRatio) {
    ...
    public List<ItemsetWithCount> getItemsetsWithSupportCount(List<ItemsetWithCount> transactions,
                                                              Double supportCount) {
    ...
    protected FPTree constructTree(List<ItemsetWithCount> transactions, int supportCount) {
...
//        log.debug("FPTree mine: {}", en - st);
    ...
    public List<ItemsetWithCount> getCounts(
            List<ItemsetWithCount> transactions,
            Map<Integer, Double> initialCounts,
