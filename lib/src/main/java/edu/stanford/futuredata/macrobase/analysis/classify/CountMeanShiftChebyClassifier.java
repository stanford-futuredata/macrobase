...
public class CountMeanShiftChebyClassifier extends Classifier {
    ...
    public static String outlierSum2ColumnName = "_OUTLIERSUM2";
    public static String inlierSum2ColumnName = "_INLIERSUM2";
     ...
     */
    public CountMeanShiftChebyClassifier(
    ...
    public void process(DataFrame input) throws Exception {
        ...
        double[] outlierSum2Column = new double[len];
        double[] inlierSum2Column = new double[len];
        for (int i = 0; i < len; i++) {
            if ((isStrPredicate && strPredicate.test(stringMetrics[i])) ||
                ...
                outlierSum2Column[i] = totalMeanColumn[i]*totalMeanColumn[i];
            } else {
                inlierCountColumn[i] = 1.0;
                inlierMeanColumn[i] = totalMeanColumn[i];
                inlierSum2Column[i] = totalMeanColumn[i]*totalMeanColumn[i];
            }
        }
        ...
        output.addColumn(outlierSum2ColumnName, outlierSum2Column);
        output.addColumn(inlierSum2ColumnName, inlierSum2Column);
    }
