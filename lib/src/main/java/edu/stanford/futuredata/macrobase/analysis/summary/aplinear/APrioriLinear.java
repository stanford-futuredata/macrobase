...
public class APrioriLinear {
    ...
    public List<APLExplanationResult> explain(
        ...
        final int maxOrder,
        int numThreads,
        Map<Integer, RoaringBitmap>[][] bitmap,
        List<Integer>[] outlierList,
        boolean[] isBitmapEncoded,
        Set<Set<Integer>> functionalDependencies
    ) {
            ...
            for (int i = 0; i < numColumns; i++) {
                if (!isBitmapEncoded[i]) {
                    for (int j = startIndex; j < endIndex; j++) {
                        byThreadAttributesTranspose[threadNum][i][j
                            - startIndex] = attributes[j][i];
                    }
                } else {
                    for (int j = 0; j < 2; j++) {
                        for (HashMap.Entry<Integer, RoaringBitmap> entry : bitmap[i][j]
                            .entrySet()) {
                        ...
                        for (int colNum = 0; colNum < numColumns; colNum++) {
                            if (isBitmapEncoded[colNum]) {
                                for (Integer curOutlierCandidate : outlierList[colNum]) {
                                    ...
                                    if (byThreadBitmap[curThreadNum][colNum][1]
                                        .containsKey(curOutlierCandidate)) {
                                        outlierCount = byThreadBitmap[curThreadNum][colNum][1]
                                            .get(curOutlierCandidate).getCardinality();
                                    }
                                    if (byThreadBitmap[curThreadNum][colNum][0]
                                        .containsKey(curOutlierCandidate)) {
                                        inlierCount = byThreadBitmap[curThreadNum][colNum][0]
                                            .get(curOutlierCandidate).getCardinality();
                                    }
                                    ...
                                    updateAggregates(thisThreadSetAggregates, curCandidate,
                                ...
                                for (int rowNum = startIndex; rowNum < endIndex; rowNum++) {
                                    ...
                                    if (useIntSetAsArray) {
                                        ...
                                        ((IntSetAsLong) curCandidate).value = curColumnAttributes[
                                            rowNum - startIndex];
                                    }
                                    updateAggregates(thisThreadSetAggregates, curCandidate,
                        ...
                        for (int colNumOne = 0; colNumOne < numColumns; colNumOne++) {
                            ...
                            for (int colNumTwo = colNumOne + 1; colNumTwo < numColumns;
                                ...
                                if (isBitmapEncoded[colNumOne] && isBitmapEncoded[colNumTwo]) {
                                    // Bitmap-Bitmap
                                    allTwoBitmap(thisThreadSetAggregates, outlierList,
                                        byThreadBitmap[curThreadNum],
                                        colNumOne, colNumTwo, useIntSetAsArray, curCandidate,
                                        numAggregates);
                                } else if (isBitmapEncoded[colNumOne]
                                    && !isBitmapEncoded[colNumTwo]) {
                                    // Bitmap-Normal
                                    allOneBitmapOneNormal(thisThreadSetAggregates,
                                        outlierList[colNumOne],
                                        byThreadBitmap[curThreadNum][colNumOne],
                                    ...
                                    // Normal-Bitmap
                                    allOneBitmapOneNormal(thisThreadSetAggregates,
                                        outlierList[colNumTwo],
                                        byThreadBitmap[curThreadNum][colNumTwo],
                                        curColumnOneAttributes, startIndex, useIntSetAsArray,
                                        curCandidate, numAggregates);
                                } else {
                                    // Normal-Normal
                            ...
                            for (int colNumTwo = colNumOne + 1; colNumTwo < numColumns;
                                ...
                                for (int colNumThree = colNumTwo + 1; colNumThree < numColumns;
                                    ...
                                    if (isBitmapEncoded[colNumOne] && isBitmapEncoded[colNumTwo]
                                        ...
                                        allThreeBitmap(thisThreadSetAggregates, outlierList,
                                            byThreadBitmap[curThreadNum],
                                            colNumOne, colNumTwo, colNumThree, useIntSetAsArray,
                                            curCandidate, numAggregates);
                                        ...
                                        allTwoBitmapsOneNormal(thisThreadSetAggregates, outlierList,
                                            byThreadBitmap[curThreadNum], colNumOne, colNumTwo,
                                            curColumnThreeAttributes, startIndex, useIntSetAsArray,
                                            curCandidate, numAggregates);
                                        ...
                                        allTwoBitmapsOneNormal(thisThreadSetAggregates, outlierList,
                                            byThreadBitmap[curThreadNum], colNumOne, colNumThree,
                                            curColumnTwoAttributes, startIndex, useIntSetAsArray,
                                            curCandidate, numAggregates);
                                        ...
                                        allTwoBitmapsOneNormal(thisThreadSetAggregates, outlierList,
                                            byThreadBitmap[curThreadNum], colNumTwo, colNumThree,
                                            curColumnOneAttributes, startIndex, useIntSetAsArray,
                                            curCandidate, numAggregates);
                                        ...
                                        allOneBitmapTwoNormal(thisThreadSetAggregates,
                                            outlierList[colNumOne],
                                            byThreadBitmap[curThreadNum][colNumOne],
                                        ...
                                        allThreeNormal(thisThreadSetAggregates,
                                            curColumnOneAttributes, curColumnTwoAttributes,
                                            curColumnThreeAttributes,
                                            startIndex, endIndex, useIntSetAsArray, curCandidate,
                                            aRows, numAggregates);
                                    }
                                }
                            }
    ...
    private void updateAggregates(FastFixedHashTable thisThreadSetAggregates, IntSet curCandidate,
    ...
    private void allOneBitmapOneNormal(FastFixedHashTable thisThreadSetAggregates,
        List<Integer> outlierColList,
        Map<Integer, RoaringBitmap>[] byThreadColumnBitmap,
        ...
        for (Integer curCandidateOne : outlierColList) {
            if (curCandidateOne == AttributeEncoder.noSupport
                || !singleNextArray[curCandidateOne]) {
                continue;
            }
    ...
    private void oneBitmapOneNormal(FastFixedHashTable thisThreadSetAggregates,
        ...
        for (Integer rowNum : bitmap) {
            ...
            // Cascade to arrays if necessary, but otherwise pack attributes into longs.
            if (useIntSetAsArray) {
                curCandidate = new IntSetAsArray(curCandidateOne,
                    curColumnTwoAttributes[rowNumInCol]);
            } else {
                ((IntSetAsLong) curCandidate).value = IntSetAsLong
                    .twoIntToLong(curCandidateOne, curColumnTwoAttributes[rowNumInCol]);
            }
    ...
    private void allTwoBitmap(FastFixedHashTable thisThreadSetAggregates,
        List<Integer>[] outlierList,
        Map<Integer, RoaringBitmap>[][] byThreadBitmap,
        ...
        for (Integer curCandidateOne : outlierList[colNumOne]) {
            ...
            for (Integer curCandidateTwo : outlierList[colNumTwo]) {
                if (curCandidateTwo == AttributeEncoder.noSupport
                    || !singleNextArray[curCandidateTwo]) {
                    continue;
                }
                // Cascade to arrays if necessary, but otherwise pack attributes into longs.
                if (useIntSetAsArray) {
                ...
                if (byThreadBitmap[colNumOne][1].containsKey(curCandidateOne) &&
                    ...
                    outlierCount = RoaringBitmap
                        .andCardinality(byThreadBitmap[colNumOne][1].get(curCandidateOne),
                ...
                if (byThreadBitmap[colNumOne][0].containsKey(curCandidateOne) &&
                    ...
                    inlierCount = RoaringBitmap
                        .andCardinality(byThreadBitmap[colNumOne][0].get(curCandidateOne),
        ...
        for (Integer curCandidateOne : outlierColList) {
            ...
            if (byThreadColumnBitmap[1].containsKey(curCandidateOne)) {
                ...
                oneBitmapTwoNormal(thisThreadSetAggregates, outlierBitmap, curCandidateOne,
                    curColumnTwoAttributes, curColumnThreeAttributes, startIndex, useIntSetAsArray,
        ...
        for (Integer curCandidateOne : outlierList[colNumOne]) {
            ...
            for (Integer curCandidateTwo : outlierList[colNumTwo]) {
                ...
                if (byThreadColumnBitmap[colNumOne][1].containsKey(curCandidateOne)
                    ...
                    RoaringBitmap outlierBitmap = RoaringBitmap
                        .and(byThreadColumnBitmap[colNumOne][1].get(curCandidateOne),
                            byThreadColumnBitmap[colNumTwo][1].get(curCandidateTwo));
                    twoBitmapsOneNormal(thisThreadSetAggregates, outlierBitmap, curCandidateOne,
                        curCandidateTwo,
                ...
                if (byThreadColumnBitmap[colNumOne][0].containsKey(curCandidateOne)
                    ...
                    RoaringBitmap inlierBitmap = RoaringBitmap
                        .and(byThreadColumnBitmap[colNumOne][0].get(curCandidateOne),
                            byThreadColumnBitmap[colNumTwo][0].get(curCandidateTwo));
                    twoBitmapsOneNormal(thisThreadSetAggregates, inlierBitmap, curCandidateOne,
                        curCandidateTwo,
                        curColumnThreeAttributes, startIndex, useIntSetAsArray, curCandidate,
                        new double[]{0, 1}, numAggregates);
                }
        ...
        for (Integer curCandidateOne : outlierList[colNumOne]) {
            ...
            for (Integer curCandidateTwo : outlierList[colNumTwo]) {
                ...
                for (Integer curCandidateThree : outlierList[colNumThree]) {
                    if (curCandidateThree == AttributeEncoder.noSupport
                        || !singleNextArray[curCandidateThree]) {
                        continue;
                    }
                    // Cascade to arrays if necessary, but otherwise pack attributes into longs.
                    if (useIntSetAsArray) {
