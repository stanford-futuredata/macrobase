...
public class ModBitSet implements Cloneable, java.io.Serializable {
    ...
    public static ModBitSet valueOf(long[] longs) {
        ...
        for (n = longs.length; n > 0 && longs[n - 1] == 0; n--)
            ;
        return new ModBitSet(Arrays.copyOf(longs, n));
    }
    ...
    public static ModBitSet valueOf(byte[] bytes) {
        return ModBitSet.valueOf(ByteBuffer.wrap(bytes));
    }
    ...
    public ModBitSet get(int fromIndex, int toIndex) {
        ...
        if (len <= fromIndex || fromIndex == toIndex)
            return new ModBitSet(0);

        // An optimization
        if (toIndex > len)
            toIndex = len;

        ModBitSet result = new ModBitSet(toIndex - fromIndex);
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof ModBitSet))
            return false;
        if (this == obj)
            return true;

        ModBitSet set = (ModBitSet) obj;
    ...
    public Object clone() {
        ...
        try {
            ModBitSet result = (ModBitSet) super.clone();
