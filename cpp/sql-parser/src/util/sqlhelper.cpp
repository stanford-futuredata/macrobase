...
namespace hsql {
  ...
  void printTableRefInfo(TableRef* table, uintmax_t numIndent) {
    ...
    case kTableCrossProduct:
      ...
      break;
    case kTableDiff:
      // Print out Macrodiff query
      inprint("Diff", numIndent);
      inprint("First", numIndent + 1);
      printTableRefInfo(table->diff->first, numIndent + 2);
      inprint("Second", numIndent + 1);
      if (table->diff->second != nullptr) {
        printTableRefInfo(table->diff->second, numIndent + 2);
      } else {
        inprint("None", numIndent + 2);
      }
      inprint("On:", numIndent + 1);
      for (Expr* expr : *table->diff->attribute_cols)
        printExpression(expr, numIndent + 2);
      inprint("COMPARE BY:", numIndent + 1);
      printExpression(table->diff->compare_by, numIndent + 2);
      inprint("MAX COMBO:", numIndent + 1);
      printExpression(table->diff->max_combo, numIndent + 2);
      break;
    }
