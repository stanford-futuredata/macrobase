...
%token CASCADE COLUMNS COMPARE CONTROL DEFAULT EXECUTE
...
%token ARRAY COMBO CROSS DELTA GROUP INDEX INNER LIMIT LOCAL
%token MERGE MINUS ORDER OUTER RIGHT TABLE UNION USING WHERE
%token CALL CASE DATE DESC DIFF DROP ELSE FILE FROM FULL
%token HASH HINT INTO JOIN LEFT LIKE LOAD NULL PART PLAN
%token SHOW TEXT THEN TIME VIEW WHEN WITH ADD ALL AND ASC
%token CSV END FOR INT KEY MAX NOT OFF SET TBL TOP AS BY IF
...
%type <table> 		from_clause table_ref table_ref_diff table_ref_atomic table_ref_name nonjoin_table_ref_atomic
%type <table>		join_clause table_ref_name_no_alias
%type <expr> 		expr operand scalar_expr unary_expr binary_expr logic_expr exists_expr
%type <expr>		function_expr between_expr expr_alias param_expr opt_max_combo
 ...
 * INSERT INTO employees SELECT * FROM students
 ******************************/
insert_statement:
...
/******************************
 * Macrodiff definition
 ******************************/
table_ref_diff:
                /* two relations */
                DIFF table_ref_atomic ',' table_ref_atomic ON expr_list COMPARE BY function_expr opt_max_combo {
                  $$ = new TableRef(kTableDiff);
                  $$->diff = new DiffDefinition();
                  $$->diff->first = $2;
                  $$->diff->second = $4;
                  $$->diff->attribute_cols = $6;
                  $$->diff->compare_by = $9;
                  $$->diff->max_combo = $10;
                }
                /* one relation */
	|	DIFF table_ref_atomic ON expr_list COMPARE BY function_expr opt_max_combo {
                  $$ = new TableRef(kTableDiff);
                  $$->diff = new DiffDefinition();
                  $$->diff->first = $2;
                  $$->diff->second = nullptr;
                  $$->diff->attribute_cols = $4;
                  $$->diff->compare_by = $7;
                  $$->diff->max_combo = $8;
                }
        ;
...
opt_max_combo:
		MAX COMBO int_literal { $$ = $3; }
	|	/* empty */ { $$ = Expr::makeLiteral((int64_t) 3); }
	;
