...
class MultiMAD:
	...
	def __init__(self, use_percentile=True):
		...
		self.low_cutoff = 0
		self.high_cutoff = 0
		self.min_support = 0.005
		self.min_risk_ratio = 3
		self.detect_low = True
		self.detect_high = True
		self.explanations = []
		self.ranking_top_ratio = []
		self.ranking_num_attrs = []
		self.explanation_attr_by_col = []
			...
			for i, result in enumerate(column):
				if result == 1:
					...
					if attr in outlier_counts:
						...
						outlier_counts[attr] = 1
			explanation_attrs = []
			support_required = int(self.min_support * num_outliers)
			num_attrs = 0
			top_ratio = 0
			print "Col {}: Total outliers: {}".format(col_no, num_outliers)
			col_no += 1
			for key, count in outlier_counts.iteritems():
				...
				if risk_ratio < self.min_risk_ratio:
					continue
				self.explanations.append(Explanation(key, risk_ratio))
				explanation_attrs.append(key)
				num_attrs += 1
			self.ranking_num_attrs.append(num_attrs)
			self.ranking_top_ratio.append(top_ratio)
			self.explanation_attr_by_col.append(explanation_attrs)
	...
	def compute_mad(self, metrics):
		...
		if MAD == 0:
			...
			MAD = np.sum(residuals[0.05*length:-0.05*length])/(0.9*length)
		# print "median: {}, MAD: {}".format(median, MAD)
		MAD *= (MultiMAD.MAD_TO_ZSCORE_COEFFICIENT * self.cutoff)
