...
/**
 * "$INTERNAL$BIND"(value, targetFunction) <p> When invoked, the returned function inserts the given
 * value as the leading argument to the targetFunction.  The other arguments are passed unchanged.
 * <p> Bind is fundamentally first class, just like function applications. It requires special
 * treatment from the type system. There is no way to write out the type of `bind`. The return type
 * of `bind` is function. Its type looks somewhat like: <p><ul> <li>X, (X) -> Y => () -> Y <li>X1,
 * (X1, X2) -> Y => (X2) -> Y <li>X1, (X1, X2, X3) -> Y => (X2, X3) -> Y <li>X1, (X1, X2, X3, X4) ->
...
public class BindExpression
  ...
  private BindExpression(Optional<NodeLocation> location, List<Expression> values,
      Expression function) {
    super(location);
