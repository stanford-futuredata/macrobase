...
class AstBuilder
  ...
  public Node visitLike(SqlBaseParser.LikeContext context) {
    ...
    Expression result = new LikePredicate(getLocation(context), (Expression) visit(context.value),
  ...
  public Node visitNormalize(SqlBaseParser.NormalizeContext context) {
    ...
    String normalForm = Optional.ofNullable(context.normalForm()).map(ParserRuleContext::getText)
        .orElse("NFC");
  ...
  public Node visitFunctionCall(SqlBaseParser.FunctionCallContext context) {
    ...
    if (name.toString().equalsIgnoreCase("try")) {
      check(context.expression().size() == 1, "The 'try' function must have exactly one argument",
          context);
    ...
    for (int i = 0; i < rawContent.length(); i++) {
      ...
      switch (state) {
        ...
        case UNICODE_SEQUENCE:
          check(isHexDigit(ch), "Incomplete escape sequence: " + escapedCharacterBuilder.toString(),
              context);
          escapedCharacterBuilder.append(ch);
